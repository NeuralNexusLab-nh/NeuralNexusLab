...
class NXClawConfig:
    def __init__(self, workspace=None):
        # Default workspace is inside the user's home directory under a folder named NXClaw
...
"""
    ...
    def banner(cls):
        ...
        print(cls.c(f"  v{VERSION}  |  zero-dep  |  {platform.system()}", cls.GRAY))
        print()
...
class Spinner:
    # Clockwise rotating triangular Braille character animation
    FRAMES = ["⠇", "⠋", "⠙", "⠸", "⠴", "⠦"]
...
class InterruptibleCall:
    ...
    def run(self, poll_interval=0.1):
        """
        Runs the call in a daemon thread. Detects Ctrl+C. 
        Requires pressing Ctrl+C twice within 2.5 seconds to confirm cancellation.
        """
    ...
    def browse_web(self, url):
        ...
        try:
            req = urllib.request.Request(
                ...
                headers={
                    "User-Agent": (
                        "Mozilla/5.0 (compatible; NXClaw/1.4; "
                        "+stdlib-only-agent)"
                    )
                },
            )
...
def run_setup_menu(config: NXClawConfig, ui: NXClawUI, is_reconfigure=False):
    ...
    print()
    max_iter_default = str(config.data.get("max_iterations", 100))
    while True:
        max_iter_str = prompt_text("  Maximum Tool Iterations Limit", default=max_iter_default, ui=ui)
    ...
    ui.success_box(
        ...
        f"Max Iterations:   {config.data['max_iterations']}\n"
...
def print_help(ui: NXClawUI):
    ...
    commands = (
        "/settings       Re-configure backend properties (provider, endpoint, key, model, limits)\n"
    ...
    tips = (
        ...
        "• Press Ctrl+C twice inside 2.5 seconds to interrupt reasoning streams immediately.\n"
...
def run_tutorial(config: NXClawConfig, ui: NXClawUI):
    ...
    steps = [
        ...
        (
            ...
            "The model you choose MUST support structured tool calling / function calling.\n"
            "If it does not, the agent may run endlessly or only reply in plain text without performing actions.",
        ),
        (
...
def main():
    ...
    is_first_run = not config.exists()
    ...
    print()
    ui.hr(color=ui.GREEN)
    print(ui.c("  Enter your request, or type /help for commands. Use Ctrl+C twice to stop streams.", ui.GRAY))
    ui.hr(color=ui.GREEN)
    print()
